pipeline {
    ...
    stages {
        ...
        stage('Build Stage') {
            ...
            post {
                success {
                    script {
                        def server = Artifactory.newServer(url: 'http://13.233.83.12:8081/artifactory', credentialsId: 'jfrog')
	...
	stage('Push to ECR and Deply to eks') {
            ...
            steps {
                script {
                    sh 'curl -o jenkins-test-2.0.jar http://13.233.83.12:8081/artifactory/libs-release/com/example/jenkins-test/2.0/jenkins-test-2.0.jar'
                    sh '''
